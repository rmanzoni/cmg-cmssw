...
double scaleIpVarslinearMC(double ipvar, int pdgId, double pt, double eta, int mcMatchId, int mcMatchAny) {
    if (abs(pdgId) == 13) {
        if (mcMatchId > 0 || mcMatchAny <= 1) {
...
double scaleSip3dMC(double sip3d, int pdgId, double pt, double eta, int mcMatchId, int mcMatchAny) {
    ...
    return scaleIpVarslinearMC(sip3d,pdgId,pt,eta,mcMatchId,mcMatchAny);
}
